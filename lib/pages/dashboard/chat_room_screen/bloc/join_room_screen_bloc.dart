...
part 'join_room_screen_event.dart';
part 'join_room_screen_state.dart';

class JoinRoomScreenBloc
    extends Bloc<JoinRoomScreenEvent, JoinRoomScreenState> {
  JoinRoomScreenBloc(String channel) : super(JoinRoomScreenState.empty()) {
    add(InitializeChatRoomScreen(channel: channel));
  }

  @override
  Stream<JoinRoomScreenState> mapEventToState(
    JoinRoomScreenEvent event,
  ) async* {
  ...
  Stream<JoinRoomScreenState> _mapInitializeChatRoomScreenToState(
    ...
    _engine.setAudioProfile(AudioProfile.SpeechStandard, AudioScenario.Default);
    ...
    _engine.adjustAudioMixingVolume(100);

    await _engine.joinChannel(null, event.channel, null, 0);
    yield state.copyWith(loading: false);
  }

  Stream<JoinRoomScreenState> _mapAgoraEngineErrorToState(
  ...
  Stream<JoinRoomScreenState> _mapAgoraEngineLeaveChannelToState(
  ...
  Stream<JoinRoomScreenState> _mapAgoraEngineFirstRemoteVideoFrameToState(
  ...
  Stream<JoinRoomScreenState> _mapAgoraEngineUserOfflineToState(
  ...
  Stream<JoinRoomScreenState> _mapAgoraEngineUserJoinedToState(
  ...
  Stream<JoinRoomScreenState> _mapAgoraEngineJoinChannelSuccessToState(
  ...
  Stream<JoinRoomScreenState> _mapToggleMuteToState(ToggleMute event) async* {
  ...
  Stream<JoinRoomScreenState> _mapSwitchCameraToState(
